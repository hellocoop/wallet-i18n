...
| de | German | Complete | [Zak Greant](https://github.com/zakgreant), Nicole Fior-Greant
...
While localising, you'll come across keys like `Continue with {provider}` and `Welcome {name}`.
The content inside the curly brackets should **not** be translated, neither should "Hellō".
...
`<br/>` - Indicates a line break; the text following this would be pushed to the next line. When localising, please add `<br>` where necessary.
